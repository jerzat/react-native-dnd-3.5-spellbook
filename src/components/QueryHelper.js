class QueryHelper {
    ...
    static async searchQuery(state) {
        ...
        var populateRecords = async (results) => { // Async, will wait for multiple querys
            ...
            //(async function queryLoop() {
                ...
                for (let i = 0; i < results.rows.length; i++) {
                    ...
                    promises.push(state.db.executeSql(queryString));
                }

            await Promise.all(promises).then(function (values) {
                newRecords = values.map( ([singleSpellResults]) => consolidate(singleSpellResults));
            });
            return newRecords;
        ...
        await state.db.executeSql(queryString).then( async ([results]) => {
            console.log("Query completed");
            newRecords = await populateRecords(results);
        });
        return discardOldVersions(newRecords); // Discard duplicates (due to spells published in more than one manual)
    }
